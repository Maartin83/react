...
const GestionCitas = () => {
    ...
    useEffect(() => {
        ...
        axios.get('http://localhost:8080/api/citas/getall',returnToken())
            .then((response) => {
    ...
    const handleEliminarCita = async (citaId) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                const responde=await fetch(`http://localhost:8080/api/citas/cancelar/${citaId}`,{
                    method: 'PUT',
                    headers: {
                    ...
                    <table className="table">
                        ...
                        {citas.map((cita) => (
                            <tr key={cita.id}>
                                ...
                                <td colSpan={5}>
                                    <Link className="btn btn-secondary mx-2" to={"/ver-pdf/?id="+cita.id} target={"_blank"}>Info Pago</Link>
