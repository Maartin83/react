...
function EditarDoctor(){
    ...
    useEffect(() => {
        ...
        async function fetchDoctorData() {
            try {
                const response = await axios.get(`http://localhost:8080/api/doctor/get/${id}`,returnToken());
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response=await axios.put(`http://localhost:8080/api/doctor/update`, doctor,returnTokenJson());
            const data = response.data;
            if (data.id){
                await Swal.fire({
