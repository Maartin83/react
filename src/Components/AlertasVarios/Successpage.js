...
function SuccessPage() {
    ...
    async function obtenerPago(id) {
        try {
            const response = await axios.get(
                `http://localhost:8080/api/pagos/getpago/${id}`,
                returnToken()
            );
            if (paymentId === null) {
    ...
    async function registrarCita() {
        if (!citaRegistradaRef.current) {
            ...
            try {
                axios.post(
                    `http://localhost:8080/api/citas/create`,
                    cita,
                    returnTokenJson()
                ).then((response) => {
                    citaId = response.data.id;
                    axios.delete(`http://localhost:8080/api/horario/disable/${cita.horario.id}`, returnToken()).then((r) => console.log(r));
        ...
        if (!pagoRegistradoRef.current) {
            ...
            try {
                const response = await axios.post(
                    `http://localhost:8080/api/pagos/create`, {
                        "id": "",
                        "codigo": pago.id,
                        "cita": {
                            "id": citaId
